...
{
    /// <summary>Maximale Anzahl der punktgetrennten Kernkomponenten (Major.Minor.Patch).</summary>
    private const int MaxVersionComponents = 3;

    /// <summary>Array-Index der Patch-Komponente in der geparsten Zahlenliste.</summary>
    ...
    public static bool TryParse(string? text, out SemanticVersion version)
    {
        ...
        string core = StripBuildMetadata(NormalizeLeadingV(text.Trim()));
        if (!TrySplitPreRelease(ref core, out string? preRelease))
        {
            return false;
        }

        if (!TryParseCoreNumbers(core, out int[] numbers))
        {
            return false;
        }

        version = new SemanticVersion(numbers[0], numbers[1], numbers[PatchIndex], preRelease);
        return true;
    }
    ...
    /// <summary>Entfernt ein optionales führendes <c>v</c>/<c>V</c> (z. B. aus einem Git-Tag).</summary>
    private static string NormalizeLeadingV(string value) =>
        value.StartsWith('v') || value.StartsWith('V') ? value[1..] : value;

    /// <summary>Verwirft SemVer-Build-Metadaten nach <c>+</c> (für die Präzedenz irrelevant).</summary>
    ...
    /// <summary>
    /// Trennt einen optionalen Pre-Release-Suffix nach <c>-</c> ab. Kürzt <paramref name="core"/>
    /// auf den Kernteil. Gibt <see langword="false"/> zurück, wenn der Suffix leer ist.
    /// </summary>
    private static bool TrySplitPreRelease(ref string core, out string? preRelease)
    {
        preRelease = null;
        int dashIndex = core.IndexOf('-', StringComparison.Ordinal);
        if (dashIndex < 0)
        {
            return true;
        }
    ...
    /// <summary>Parst die punktgetrennten Kernkomponenten; fehlende werden mit 0 aufgefüllt.</summary>
    private static bool TryParseCoreNumbers(string core, out int[] numbers)
    {
        numbers = [0, 0, 0];
        string[] parts = core.Split('.');
        if (parts.Length is < 1 or > MaxVersionComponents)
        {
            return false;
        }

...
public interface IMarkdownDocumentRepository
{
    ...
    /// <summary>
    /// Lädt die vorhandenen Dokumente zu den angegebenen MarkdownFile-Ids in einem gechunkten,
    /// getrackten Batch. Fehlende Ids fehlen im Ergebnis; die Reihenfolge ist nicht garantiert.
    /// </summary>
    /// <param name="markdownFileIds">Gesuchte <c>MarkdownFile.Id</c>s.</param>
    /// <param name="cancellationToken">Abbruchsteuerung.</param>
    Task<IReadOnlyDictionary<Guid, MarkdownDocument>> GetByMarkdownFileIdsAsync(

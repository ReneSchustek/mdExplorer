        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            foreach (Guid[] chunk in ids.Chunk(SqliteInListBatchSize))
            {
                ...
                await LoadBodyChunkAsync(connection, chunk, result, cancellationToken).ConfigureAwait(false);
            }
            return result;
        }
        finally
...
#pragma warning restore CA2100
        for (int i = 0; i < chunk.Length; i++)
        {
            ...
            _ = command.Parameters.AddWithValue(parameterName, FormatGuid(chunk[i]));
        }
        ...
        while (await reader.ReadAsync(cancellationToken).ConfigureAwait(false))
        {
            ...
            Guid id = Guid.Parse(reader.GetString(0), CultureInfo.InvariantCulture);
            string body = await reader.IsDBNullAsync(1, cancellationToken).ConfigureAwait(false)
                ? string.Empty
                : reader.GetString(1);
            sink[id] = body;
        }
    ...
    private static string BuildBodiesByIdsSql(int parameterCount)
    {
        ...
        _ = sql.Append(SelectBodiesByIdsPrefix);

...
public sealed class TagNormalizerTests
{
    ...
    [InlineData("MünchenÖlÜbung", "münchenölübung")]
    [InlineData("Tag42", "tag42")]
    public void TryToSlug_OnValidInput_ReturnsTrueWithSlug(string input, string expected)
    {
        bool ok = _sut.TryToSlug(input, out string slug);
    ...
    [InlineData("+")]
    [InlineData("...,,,")]
    public void TryToSlug_OnNonSluggableInput_ReturnsFalseWithEmptySlug(string? input)
    {
        bool ok = _sut.TryToSlug(input, out string slug);
